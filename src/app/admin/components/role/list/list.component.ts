...
export class ListComponent extends BaseComponent implements OnInit {
  ...
  async getRoles() {
    ...
    const allRoles: { datas: ListRole[], totalCount: number } = await this.roleService.getRoles(this.paginator ? this.paginator.pageIndex : 0, this.paginator ? this.paginator.pageSize : 5, () => this.hideSpinner(SpinnerType.Cog), errorMessage => this.alertifyService.message(errorMessage, {
      dismissothers: true,
